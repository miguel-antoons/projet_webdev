...
function ProjetsFactures () {
    ...
    useEffect(() => {
        ...
        const fetchContent = async () => {
            try {
                const response = await fetch('/api/facture?filter=' + filter);
    ...
    const sortContent = (arrayToSort) => {
        ...
        if (values["sign"]) {
            sortedContent.sort((a, b) => {
                ...
                if (values["key"] === "date" || values["key"] === "attribute2") {
...
export default ProjetsFactures;